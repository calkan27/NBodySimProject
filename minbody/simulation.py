"""
This core module implements the main NBodySimulation class that orchestrates the entire
simulation.

Key responsibilities include managing particle state (positions, velocities, masses),
coordinating integration schemes (Verlet, Yoshida4, WHFast, ham_soft), handling adaptive
softening through SofteningManager, providing state serialization and restoration, and
maintaining energy conservation tracking. The class offers multiple initialization paths
(Bodies, arrays, or restored states), interfaces with various integrators based on
configuration, supports both fixed and adaptive timestepping, and provides comprehensive
state introspection. It serves as the primary user interface for running simulations and
assumes valid initial conditions and compatible configuration settings.
"""
...
class NBodySimulation:
	...
	def snapshot(self) -> dict:
		...
		soft_state = {
			"s0": getattr(sim.manager, "s0", getattr(sim.manager, "s", 0.0)),
			"s": getattr(sim.manager, "s", 0.0),
			"s2": getattr(sim.manager, "s2", 0.0),
			"step_s2": getattr(sim.manager, "_step_s2", 0.0),
			"_step_s2": getattr(sim.manager, "_step_s2", 0.0),  
			"min_softening": getattr(sim, "_min_softening", 0.0),
		...
		print("[snapshot] softening_mgr_state keys included: " + ", ".join(sorted(list(soft_state.keys()))))
		...
		snap = {
			"masses": getattr(sim._state, "_mass", np.empty(0)),
			"positions": getattr(sim._state, "_pos", np.empty((0, 2))),
			"velocities": getattr(sim._state, "_vel", np.empty((0, 2))),
			...
			"acc": getattr(sim._state, "_acc", np.empty((0, 2))),
		}
		return snap
	...
	def restore(cls, state):
		cfg_in = state.get("cfg", state.get("sim", {}).get("cfg"))
		if cfg_in:
			cfg = cfg_in.copy()
		else:
			cfg = SimConfig()
		...
		sim_flags = state.get("sim_state", sim_data.get("flags", {}))
		...
		if isinstance(hist, (list, tuple)):
			if len(hist) > 0:
				...
				if isinstance(v0, (int, float, np.floating)):
					if v0 == v0 and v0 != float("inf") and v0 != float("-inf"):
						s0_snap = float(v0)
		if s0_snap is None:
			val = state.get("softening", soft_data.get("s", 1.0e-3))
		...
		if isinstance(sim_flags, dict):
			if "_min_softening" in sim_flags:
				ms = sim_flags["_min_softening"]
				if isinstance(ms, (int, float, np.floating)):
					min_snap = float(ms)
		if min_snap is None:
			ms2 = state.get("min_softening")
			if isinstance(ms2, (int, float, np.floating)):
				min_snap = float(ms2)
		if min_snap is None:
			if s0_snap > 0.0:
				min_snap = 0.1 * s0_snap
			else:
				min_snap = 0.0

		if isinstance(sim_flags, dict):
			adaptive_ts = bool(sim_flags.get("_adaptive_timestep", False))
		else:
			adaptive_ts = False
		if isinstance(sim_flags, dict):
			adaptive_soft = bool(sim_flags.get("_adaptive_softening", False))
		else:
			adaptive_soft = False

		if hasattr(cfg, "integrator_mode"):
			mode_pass = cfg.integrator_mode
		else:
			mode_pass = None
		...
		sim = cls(
			...
			softening=float(s0_snap),
			min_softening=float(min_snap),
			adaptive_timestep=adaptive_ts,
			adaptive_softening=adaptive_soft,
			skip_init_corrector=True,
			skip_cm_recenter=True,
			integrator_mode=mode_pass,
		)

		SimulationState.restore_to_sim(state, sim)

		if isinstance(sim_flags, dict):
			if "_epsilon" in sim_flags:
				eps_now = float(sim_flags.get("_epsilon", sim.manager.s))
			else:
				eps_now = float(sim.manager.s)
		else:
			eps_now = float(sim.manager.s)

		sim.manager.update_continuous(float(eps_now))

		sim._max_softening = 10.0 * float(sim.manager.s0)
	...
	def set_softening_bounds(
		self,
		eps_min: float,
		eps_max: float,
		*,
		clamp_epsilon: bool = True,
		reset_pi_on_clamp: bool = True,
	) -> None:
		...
		self._min_softening = float(a)
		self._max_softening = float(b)

		if clamp_epsilon:
			eps_now = float(getattr(self, "_epsilon", self.manager.s))
			new_eps = float(eps_now)
			clamped = False

			if new_eps < self._min_softening:
				new_eps = float(self._min_softening)
				clamped = True
			elif new_eps > self._max_softening:
				new_eps = float(self._max_softening)
				clamped = True

			if clamped:
				self._epsilon = float(new_eps)
				if reset_pi_on_clamp:
					self._pi = float(-float(getattr(self, "_pi", 0.0)))
				self.manager.update_continuous(self._epsilon)
